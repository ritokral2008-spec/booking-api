...
{
    public class ValidateCreateRoomDto:
        AbstractValidator<CreateRoomDto>
    {
        public ValidateCreateRoomDto()
        {
            RuleFor(x => x.Title)
                .NotEmpty()
                .Must(x => !string.IsNullOrWhiteSpace(x))
                .WithMessage("Название комнаты не можеть быть пустым или состоять исключительно из пробелов")
                .MaximumLength(100)
            ...
            RuleFor(x => x.Address)
                ...
                .MaximumLength(300)
                .WithMessage("Длина адреса не может превышать 300 символов");
